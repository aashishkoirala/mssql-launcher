...
func TestSqlCli_GetCommand_Works_For_Linux_Non_Integrated(t *testing.T) {
	c := &config.Connection{
		Name:     "test",
		Server:   "server1",
		Database: "database1",
...
func TestSqlCli_GetCommand_Works_For_Windows_Non_Integrated(t *testing.T) {
	c := &config.Connection{
		Name:     "test",
		Server:   "server2",
		Database: "database2",
	...
	const (
		...
		expectedArgs string = "/c python -m mssqlcli.main -S server2 -d database2 -U user2 -P password2"
	)
	if cmd != expectedCmd {
		t.Fatalf("Expected cmd = %s, got %s", expectedCmd, cmd)
	}
...
func TestSqlCli_GetCommand_Works_For_Windows_Integrated(t *testing.T) {
	c := &config.Connection{
		Name:       "test",
		Server:     "server3",
		Database:   "database3",
	...
	const (
		expectedCmd  string = "cmd"
		expectedArgs string = "/c python -m mssqlcli.main -S server3 -d database3 -E"
	)
	if cmd != expectedCmd {
		t.Fatalf("Expected cmd = %s, got %s", expectedCmd, cmd)
	}
